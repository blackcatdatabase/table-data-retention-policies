...
namespace BlackCat\Database\Packages\DataRetentionPolicies\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\DataRetentionPolicies\Dto\DataRetentionPolicyDto;
use BlackCat\Database\Packages\DataRetentionPolicies\Mapper\DataRetentionPolicyDtoMapper;
 ...
 */
final class DataRetentionPoliciesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private DataRetentionPolicyRepository $dataRetentionPolicyRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'data_retention_policies' => $this->dataRetentionPolicyRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\DataRetentionPolicies\ModuleException("Record {$idStr} not found for locking.");
            }
