...
use BlackCat\Database\Packages\DataRetentionPolicies\Repository\DataRetentionPolicyRepositoryInterface;
...
class DataRetentionPolicyRepository implements DataRetentionPolicyRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'entity_table', 'field_name', 'action', 'keep_for' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\DataRetentionPolicies\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'active', 'notes' ]);
          return count($rows);
        }
    ...
    public function getByEntityTableAndFieldNameAndActionAndKeepFor(string $entityTable, string $fieldName, string $action, string $keepFor, bool $asDto = false): array|\BlackCat\Database\Packages\DataRetentionPolicies\Dto\DataRetentionPolicyDto|null {
        return $this->getByUnique([ 'entity_table' => $entityTable, 'field_name' => $fieldName, 'action' => $action, 'keep_for' => $keepFor ], $asDto);
    }
    ...
    public function getIdByEntityTableAndFieldNameAndActionAndKeepFor(string $entityTable, string $fieldName, string $action, string $keepFor) {
        $row = $this->getByEntityTableAndFieldNameAndActionAndKeepFor($entityTable, $fieldName, $action, $keepFor, false);
