...
namespace BlackCat\Database\Packages\DataRetentionPolicies\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\DataRetentionPolicies\Definitions;
use BlackCat\Database\Packages\DataRetentionPolicies\Criteria;
use BlackCat\Database\Packages\DataRetentionPolicies\Dto\DataRetentionPolicyDto as Dto;
use BlackCat\Database\Packages\DataRetentionPolicies\Mapper\DataRetentionPolicyDtoMapper as RowMapper;
...
final class DataRetentionPolicyRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\DataRetentionPolicies\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'entity_table', 'field_name', 'action', 'keep_for' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\DataRetentionPolicies\Definitions::table());
            return $bulk->upsertMany($rows, [ 'entity_table', 'field_name', 'action', 'keep_for' ], [ 'active', 'notes' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\DataRetentionPolicies\Dto\DataRetentionPolicyDto|null */
    public function getByEntityTableAndFieldNameAndActionAndKeepFor(string $entityTable, string $fieldName, string $action, string $keepFor, bool $asDto = false): array|\BlackCat\Database\Packages\DataRetentionPolicies\Dto\DataRetentionPolicyDto|null {
        $row = $this->getByUnique([ 'entity_table' => $entityTable, 'field_name' => $fieldName, 'action' => $action, 'keep_for' => $keepFor ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\DataRetentionPolicies\Mapper\DataRetentionPolicyDtoMapper::fromRow($row);
    }
    public function existsByEntityTableAndFieldNameAndActionAndKeepFor(string $entityTable, string $fieldName, string $action, string $keepFor): bool {
        $where = 't.' . Ident::q($this->db, 'entity_table') . ' = :uniq_entity_table' . ' AND ' . 't.' . Ident::q($this->db, 'field_name') . ' = :uniq_field_name' . ' AND ' . 't.' . Ident::q($this->db, 'action') . ' = :uniq_action' . ' AND ' . 't.' . Ident::q($this->db, 'keep_for') . ' = :uniq_keep_for';
        return $this->exists($where, [ 'uniq_entity_table' => $entityTable, 'uniq_field_name' => $fieldName, 'uniq_action' => $action, 'uniq_keep_for' => $keepFor ]);
    }
    /** @return int|string|null */
    public function getIdByEntityTableAndFieldNameAndActionAndKeepFor(string $entityTable, string $fieldName, string $action, string $keepFor) {
        $row = $this->getByEntityTableAndFieldNameAndActionAndKeepFor($entityTable, $fieldName, $action, $keepFor);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\DataRetentionPolicies\Dto\DataRetentionPolicyDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\DataRetentionPolicies\Dto\DataRetentionPolicyDto|null {
